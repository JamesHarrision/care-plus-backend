...
export const emergencyInfoService = {
  async upsertEmergencyInfo(userId: string, data: UpsertEmergencyInfoInput) {
  ...
  async generateQrForCurrentUser(userId: string) {
    ...
    await emergencyInfoRepository.saveQuickAccessPublicId(publicId, userId, QUICK_ACCESS_TTL_SECONDS);
