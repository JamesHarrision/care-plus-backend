...
export const emergencyInfoController = {
  upsertEmergencyInfo: async (req: AuthRequest, res: Response) => {
    try {
      const { blood_type, allergies, chronic_diseases, emergency_contacts, current_medications, notes } = req.body;
      let userID = '';
      if (req.user) userID = req.user.id;
      else if (req.quickLoginMember) userID = req.quickLoginMember?.memberId;
      const emergencyInfo = await emergencyInfoService.upsertEmergencyInfo(userID as string, {
        blood_type,
        allergies,
        chronic_diseases,
        emergency_contacts,
  ...
  getEmergencyInfoQr: async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user && !req.quickLoginMember) {
        return res.status(401).json({ status: 'error', message: 'Unauthorized' });
      }
      let userID = '';
      if (req.user) userID = req.user.id;
      else if (req.quickLoginMember) userID = req.quickLoginMember.memberId;
      const data = await emergencyInfoService.generateQrForCurrentUser(userID);
